...
async def _(event):
    ...
    if x == 2:
        ...
        xx = await event.eor(
            xx, '`"If you buy an eraser you are literally paying for your mistakes."`'
        )
    ...
    if x == 5:
        ...
        xx = await event.eor(
            xx, '`"Any video with “wait for it” in the title is simply too long."`'
        )
    ...
    if x == 11:
        ...
        xx = await event.eor(
            xx, '`"A folder is for things that you don\'t want to fold."`'
        )
    ...
    if x == 13:
        ...
        xx = await event.eor(
            xx, '`"If everything goes smoothly, you probably won\'t remember today."`'
        )
    ...
    if x == 20:
        ...
        xx = await event.eor(
            xx, '`"Technically, no one has ever been in an empty room."`'
        )
    ...
    if x == 24:
        ...
        xx = await event.eor(xx, "`\"The word 'quiet' is often said very loud.\"`")
    ...
    if x == 34:
        ...
        xx = await event.eor(
            xx, '`"By focusing on blinking, you blink slower... Same for breathing."`'
        )
    ...
    if x == 45:
        ...
        xx = await event.eor(
            xx, '`"If Greenland actually turns green, we\'re all screwed."`'
        )
    ...
    if x == 49:
        ...
        xx = await event.eor(
            xx, '`"Ask yourself why the the brain ignores the second the."`'
        )
    ...
    if x == 53:
        ...
        xx = await event.eor(xx, '`"Raising a child is training your replacement."`')
    ...
    if x == 61:
        ...
        xx = await event.eor(
            xx, '`"6 to 6:30 feels more half-an-hour than 5:50 to 6:20"`'
        )
    ...
    if x == 65:
        ...
        xx = await event.eor(
            xx, '`"A ton of whales is really only like half a whale."`'
        )
    ...
    if x == 72:
        ...
        xx = await event.eor(
            xx, '`"The most effective alarm clock is a full bladder."`'
        )

    if x == 73:

        xx = await event.eor(
            xx, '`"You probably just synchronized blinks with millions of people."`'
        )
    ...
    if x == 75:
        ...
        xx = await event.eor(
            xx, '`"Night before a day off is more satisfying than the actual day off."`'
        )

    if x == 76:

        xx = await event.eor(
            xx, '`"We put paper in a folder to keep it from folding."`'
        )

    if x == 77:

        xx = await event.eor(
            xx, '`"Somewhere, two best friends are meeting for the first time."`'
        )
    ...
    if x == 81:
        ...
        xx = await event.eor(xx, '`"Wikipedia is what the internet was meant to be."`')
    ...
    if x == 92:
        ...
        xx = await event.eor(xx, '`"Bubbles are to fish what rain is to humans."`')
    ...
    if x == 94:
        ...
        xx = await event.eor(
            xx, '`"A comma is a short pause, a coma is a long pause."`'
        )

    if x == 95:

        xx = await event.eor(
            xx, '`"Someday you will either not wake up or not go to sleep."`'
        )

    if x == 96:

        xx = await event.eor(
            xx, '`"Bermuda Triangle might be the exit portal of this simulation."`'
        )
